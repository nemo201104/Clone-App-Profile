...
public final class HostTests {
    ...
    public static void main(String[] args) throws Exception {
        ...
        check("UP_TO_DATE".equals(UpdateMetadata.evaluate(update,10002).getString("state")),"current release is up to date");
        check("UPDATE_AVAILABLE".equals(UpdateMetadata.evaluate(update,10001).getString("state")),"previous module version detects maintenance update with the migrated validator");
        ...
        rejects(()->UpdateMetadata.evaluate(obsolete,10002),"old repository owner is not accepted by the migrated updater");
        JSONObject hostile=new JSONObject(update.toString()).put("zipUrl",update.getString("zipUrl")+"/../other.zip");
        rejects(()->UpdateMetadata.evaluate(hostile,10002),"update path traversal rejected");
        rejects(()->UpdateMetadata.evaluate(new JSONObject(update.toString()).put("versionCode","10002"),10002),"noninteger update version rejected");
        ...
        try(Store s=new Store(root.toFile(),false)) {
            ...
            restored.put("targetSerial",11);
            rejects(()->{Store.validate(s.data);return null;},"recycled profile serial cannot retain old clone identity");
            restored.put("targetSerial",10);
        ...
        try(Store s=new Store(root.toFile(),false)) {
            s.data.getJSONArray("pending").put(Json.obj("operationId","interrupted-test","kind","clone","targetSerial",10));s.save();
        }
        try(Store s=new Store(root.toFile(),false)) {
            check(s.data.getJSONArray("pending").getJSONObject(0).getString("operationId").equals("interrupted-test"),"interrupted operation remains journaled across reopen");
            s.data.put("pending",new JSONArray());s.save();
        }
        ...
        Files.write(props.resolve("module.prop"),"versionCode=10002x\n".getBytes("UTF-8"));
